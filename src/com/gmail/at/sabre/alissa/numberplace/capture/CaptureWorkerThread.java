...
//	}
	...
	private static Bitmap recognize(Bitmap src_bitmap) throws Exception {
		...
		Mat source = new Mat();
		Utils.bitmapToMat(src_bitmap, source);
		
		Mat gray = new Mat();
		Imgproc.cvtColor(source, gray, Imgproc.COLOR_RGBA2GRAY);
		
		Mat binary = new Mat();
		int blockSize = (Math.min(gray.width(), gray.height()) / 18) | 1; // make the blocksize odd so that a block has a center pixel.
		Imgproc.adaptiveThreshold(gray, binary, 255, Imgproc.ADAPTIVE_THRESH_MEAN_C, Imgproc.THRESH_BINARY_INV, blockSize, 16);

		Mat result = binary;
		
		Bitmap dst_bitmap = Bitmap.createBitmap(result.width(), result.height(), Config.ARGB_8888);
		Utils.matToBitmap(result, dst_bitmap);
		return dst_bitmap;
	}
}
